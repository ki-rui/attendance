...
def searchid(table,id):
    ...
    while i<=j:
        ...
        if table.cell_value(rowx=i, colx=3)==id: return i
        elif table.cell_value(rowx=j, colx=3)==id: return j
        else:
            mid=(i+j)//2
            k=-1
            m=0
            ...
            if table.cell_value(rowx=mid, colx=3)==id: return mid
            elif table.cell_value(rowx=mid, colx=3)>id:j=mid-1
            else:i=mid+1
    return -1
        ...
        for xl in xls:
            ...
            if ind!=-1:
                ...
                if isEmpty(table_list):
                    ...
                    if tmp!=-1:
                        l.debug(id[1]+"的学号检索结果为空（有可能该学生在被检索时间段内未值班），但根据其姓名在文件另一处检索到可用信息")
                        table_list=tmp
                ls.append([table_list,lenDate(addyear(conf['Date'].split('-')[0]),d0)])
            else:
                tmp = extname(table, searchname(table, id[1]), index0, index0 + l0)
                if tmp != -1:
                    l.debug(id[1] + "的学号未出现在文件中，但根据其姓名在文件另一处检索到可用信息")
                    table_list = tmp
                    ls.append([table_list,lenDate(addyear(conf['Date'].split('-')[0]),d0)])
                else:
...
def rd_all(xls,allid):
    for ids in allid:
        if conf['isSimple']=='false': read_xls(xls, ids)
        elif conf['isSimple']=='true':simple(ids)
        else:
